...
class CodeData(Dataset):
    def __init__(self, dataset):
        ...
        if dataset == 'mimic4' : 
            self.vocab_dir = "/home/data/2025_MIMICIV_processed/mimic4/vocab.pkl"
        elif dataset == 'eicu' : 
            self.vocab_dir = "/home/data/eICU/preprocess/processed_data/eicu/vocab.pkl"
...
def main():
    ...
    embedding_save_path = os.path.join("/home/data/2025_MIMICIV_processed", "mimic4/embeddings.txt")
    save_embeddings_to_file(embeddings, embedding_save_path)

    data = CodeData(dataset="eicu")
    loader = DataLoader(data, batch_size=batch_size, shuffle=False, num_workers=num_workers)
    embeddings = get_code_embeddings(loader, model, tokenizer)
    embedding_save_path = os.path.join("/home/data/eICU/preprocess/processed_data", "eicu/embeddings.txt")
    save_embeddings_to_file(embeddings, embedding_save_path)
