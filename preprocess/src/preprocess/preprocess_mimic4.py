...
def main():
    def process_patients_admissions(infile, hosp_adm_dict):
        ...
        for line in csv.DictReader(inff):
            ...
            if discharge_status == "1":
                ...
                continue
            if line["die_in_days"] != "":
                mortality = float(line["die_in_days"]) <= target_tmp_days
            elif line["readmit_in_days"] != "" and float(line["readmit_in_days"]) > target_tmp_days:
                # no mortality info, but the patient is readmitted after @target_tmp_days days
                mortality = False
            else:
                mortality = None
            if line["readmit_in_days"] != "":
                readmission = float(line["readmit_in_days"]) <= target_tmp_days
            else:
                readmission = None
            gender = line["gender"]
            ethnicity = line["race"]
    ...
    def process_cxr(infile, hosp_adm_dict) : 
        inff = open(infile, 'r')
        count = 0
        missing_admission_id = 0
        for line in csv.DictReader(inff):
            ...
            studyid      = line["study_id"]
            
            if admission_id not in hosp_adm_dict:
                missing_admission_id += 1
                continue

            hosp_adm_dict[admission_id].cxrevents.append([timestamp, studyid, image_path])
            count += 1
        inff.close()

        print("")
        print("CXR images without admission ID: %d" % missing_admission_id)
        print("Total admission IDs processed: %d" % count)

        return hosp_adm_dict

    def post_process_cxr(hosp_adm_dict) :
        for admission_id, hosp_adm in hosp_adm_dict.items():
            if not hosp_adm.cxrevents:
                continue
            # sort by timestamp
            hosp_adm.cxrevents.sort(key=lambda x: x[0])
        return hosp_adm_dict


    # def post_process_cxrevents(hosp_adm_dict, max_len=200):
    #     # group by timestamp
    #     for admission_id, hosp_adm in hosp_adm_dict.items():
    #         # (timestamp, list of (item_id, value, flag))
    #         grouped_cxrevents = []
    #         # for timestamp, _, (itemid, valuenum, flag) in hosp_adm.cxrevents:
    #         for timestamp, studyid, image_path in hosp_adm.cxrevents:
    #             grouped_cxrevents.append((timestamp, [(studyid, image_path)]))
    #         hosp_adm.cxrevents = grouped_cxrevents
    for target_tmp_days in [7, 15, 30, 90] :
        print(f"Processing for target_tmp_days: {target_tmp_days}")
        input_path = os.path.join(processed_data_path, "mimic4")
        # output_path = os.path.join(processed_data_path, "mimic4")
        output_path = "/home/data/2025_MIMICIV_processed/mimic4_v2/"
        ...
        cxrevents_file = input_path + "/cxr_admit_tmp.csv"
        ...
        hosp_adm_dict = post_process_labevents(hosp_adm_dict, max_len=200)
        
        hosp_adm_dict = process_cxr(cxrevents_file, hosp_adm_dict)
        hosp_adm_dict = post_process_cxr(hosp_adm_dict)
        print("Processing discharge_v2_tmp.csv")
        hosp_adm_dict = process_discharge(discharge_file, hosp_adm_dict)

        dump_pickle(hosp_adm_dict, os.path.join(output_path, f"hosp_adm_dict_{target_tmp_days}days.pkl"))
        print('Saved at : ', os.path.join(output_path, f"hosp_adm_dict_{target_tmp_days}days.pkl"))
