...
class MMBackbone(nn.Module) : 
    def __init__(self, config, tokenizer) : 
        ...
        tabular_encoder = TabularEncoder(config, tokenizer)
        tab_proj = nn.Linear(config.embedding_size, config.projection_size) # embedding size is tabular embedding size
        note_encoder = TextEncoder(bert_type=config.bert_type, device='cuda')
        note_proj = nn.Linear(note_encoder.model.config.hidden_size, config.projection_size) # 128 to 512
        lab_encoder = RNNEncoder(input_size=114,
                                      ...
                                      device='cuda')
        lab_proj = nn.Linear(config.embedding_size, config.projection_size)
