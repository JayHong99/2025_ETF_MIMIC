...
class TabularEncoder(nn.Module) : 
    def __init__(self, config, tokenizer) : 
        ...
        self.pretrained_embedding = config.code_pretrained_embedding
        self.dropout = config.dropout
        self.layers = config.code_layers
        self.heads = config.code_heads
        self.device = "cuda"
        
        if self.pretrained_embedding : 
            mm = torch.Tensor(self.tokenizer.code_embeddings)
            assert self.tokenizer.code_vocabs_size == mm.size(0)
            self.code_embedding = nn.Sequential(
                nn.Embedding(self.tokenizer.code_vocabs_size, mm.shape[1]),
                nn.Linear(mm.shape[1], self.embedding_size)
            )
            self.code_embedding[0].weight.data.copy_(mm)
            self.code_embedding[0].weight.requires_grad = False
        else:
            self.code_embedding = nn.Embedding(self.tokenizer.code_vocabs_size, self.embedding_size, padding_idx=0)
        self.type_embedding = nn.Embedding(self.tokenizer.type_vocabs_size, self.embedding_size, padding_idx=0)
        self.age_embedding = nn.Embedding(self.tokenizer.age_vocabs_size, self.embedding_size, padding_idx=0)
        self.gender_embedding = nn.Embedding(self.tokenizer.gender_vocabs_size, self.embedding_size, padding_idx=0)
        self.ethnicity_embedding = nn.Embedding(self.tokenizer.ethnicity_vocabs_size, self.embedding_size, padding_idx=0)
        self.dropout_layer = nn.Dropout(self.dropout)

        self.transformer = nn.ModuleList([TransformerBlock(self.embedding_size, self.heads, self.dropout) for _ in range(self.layers)])
